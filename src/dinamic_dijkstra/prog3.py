...
transports['date'] = pd.to_datetime(transports['date'], errors='coerce')  # Convierte y maneja posibles errores

# Crear diccionario emp_type
emp_type = {node['id_emp']: node['type'] for _, node in emps.iterrows()}

# Nodo final para el análisis
source = '14159781000176'
k = 3  # Número de caminos a encontrar

# Función para crear el grafo a partir del transporte acumulativo
...
# Agrupar los datos por mes y ordenar cronológicamente
transports['year_month'] = transports['date'].dt.to_period('M')  # Ahora debería funcionar
unique_months = sorted(transports['year_month'].unique())
cumulative_data = pd.DataFrame()  # Para almacenar el camino acumulativo por cada mes

# Guardar caminos acumulativos por cada mes
monthly_paths = []

# Procesar datos acumulativos para cada mes
